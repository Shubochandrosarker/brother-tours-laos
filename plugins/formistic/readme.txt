...
suite version. It supersedes the interim `2.1.0-bt.1` label; see UPSTREAM.md
for why the number moves backward from the historical `2.1.0` entry below —
this install had never been packaged or deployed under that label.*
* Change: plugin renamed from "Brother Tours Forms" back to **Formistic**, authored by **WordPressistic**. Customer-facing forms, emails and the acknowledgement flow stay Brother Tours branded; only the plugin's own admin identity changed.
* Removed: `class-formistic-g2a-defaults.php` and every other Guns 2 Ammo-specific code path (the `g2a_request`/`g2a_reservation` legacy theme-form capture, the `wpistic_formistic_capture_g2a` option, the "Seed Guns 2 Ammo defaults" admin action, and the `g2a_biz()`/`guns2ammo` template fallbacks in the email layer). None of it is reachable from this install any longer.
* New: fifth Brother Tours form, "Request Tour Availability", with hidden `tour_id`/`tour_title` fields — see `docs/formistic-brother-tours-integration.md` for how it coexists with Tour Manager's own booking widget without double-ingesting a submission.
* Change: plugin directory and bootstrap renamed `brother-tours-formistic` → `formistic`; `WPISTIC_FORMISTIC_*` constants, `Wpistic_Formistic_*` class names, and the `wpistic_formistic_submission_captured` action are unchanged.
