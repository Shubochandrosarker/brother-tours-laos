...
class Wpistic_Formistic_Emails {
	...
	/**
	 * Live business NAP for the email footer.
	 *
	 * Sourced from the Brother Tours branding layer (configured sender name,
	 * never hard-coded), falling back to the site title when unset.
	 *
	 * @return array{name:string,addr:string,phone:string}
	 */
	protected static function biz() {
		$name = class_exists( 'Wpistic_Formistic_BT_Branding' )
			? Wpistic_Formistic_BT_Branding::from_name()
			: wp_specialchars_decode( get_bloginfo( 'name' ), ENT_QUOTES );

		return array(
			'name'  => $name,
			'addr'  => (string) get_option( 'wpistic_formistic_business_address', '' ),
			'phone' => (string) get_option( 'wpistic_formistic_business_phone', '' ),
		);
	}
	 ...
	 *
	 * The wpistic_formistic_email_logo_url option, or empty string (the header
	 * falls back to a text brand) when unset.
	 *
	 * @return string
	 */
