...
final class SessionController {
	...
	public function routes(): void {
		...
		register_rest_route(
			...
			array(
				...
				'permission_callback' => static fn( WP_REST_Request $request ) => Csrf::authorize( $request, 'bt_manage_operations', true ),
			)
		);
		register_rest_route(
			BTOA_NAMESPACE,
	...
	public function determine_current_user( $user_id ): int {
		...
		if ( ! str_contains( $uri, '/' . BTOA_NAMESPACE . '/' ) ) {
			return 0;
		}
	...
	public function login( WP_REST_Request $request ) {
		...
		if ( is_wp_error( $user ) || ! $user instanceof WP_User || ! user_can( $user, 'bt_manage_operations' ) ) {
	...
	public function session() {
		if ( ! is_user_logged_in() || ! current_user_can( 'bt_manage_operations' ) ) {
	...
	/**
	 * Restrict credentialed CORS to explicit origins for the private operations namespace only.
	 *
	...
	private function allowed_origins(): array {
		...
		if ( ! empty( $home['scheme'] ) && ! empty( $home['host'] ) ) {
			$root_host = preg_replace( '/^(www|staging)\./', '', (string) $home['host'] );
			$list[] = $home['scheme'] . '://' . $home['host'];
			$list[] = 'https://app.' . preg_replace( '/^www\./', '', (string) $home['host'] );
			$list[] = 'https://app.' . $root_host;
		}
