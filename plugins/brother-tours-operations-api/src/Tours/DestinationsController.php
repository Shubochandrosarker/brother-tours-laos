...
final class DestinationsController {
	...
	public function routes(): void {
		register_rest_route( BTOA_NAMESPACE, '/destinations', array(
			array( 'methods' => 'GET', 'callback' => array( $this, 'list' ), 'permission_callback' => static fn( WP_REST_Request $r ) => Csrf::authorize( $r, 'edit_posts', false ) ),
			array( 'methods' => 'POST', 'callback' => array( $this, 'create' ), 'permission_callback' => static fn( WP_REST_Request $r ) => Csrf::authorize( $r, 'edit_posts', true ) ),
		) );
		register_rest_route( BTOA_NAMESPACE, '/destinations/(?P<id>\d+)', array(
			array( 'methods' => 'GET', 'callback' => array( $this, 'get' ), 'permission_callback' => static fn( WP_REST_Request $r ) => Csrf::authorize( $r, 'edit_posts', false ) ),
			array( 'methods' => 'PATCH', 'callback' => array( $this, 'update' ), 'permission_callback' => static fn( WP_REST_Request $r ) => Csrf::authorize( $r, 'edit_posts', true ) ),
